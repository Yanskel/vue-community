...
export default {
  ...
  mounted() {
    ...
    this.axios.get("/echarts/epidemic").then(res => {
      ...
      this.total = jsonData.data.chinaTotal.total
      this.today = jsonData.data.chinaTotal.today
      this.extData = jsonData.data.chinaTotal.extData

      let data = [{ name: '南海诸岛', value: 0 }]
      jsonData.data.areaTree[2].children.forEach(item => {
